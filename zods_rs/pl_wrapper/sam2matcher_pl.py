...
from zods_rs.models.Sam2Matcher import Sam2Matcher
from zods_rs.models.Sam2MatchingBaseline import Sam2MatchingBaseline
from zods_rs.models.Sam2MatchingBaseline_noAMG import Sam2MatchingBaselineNoAMG

from zods_rs.dataset.metainfo import METAINFO

from zods_rs.dataset.coco_ref_dataset import (
    COCORefTrainDataset,
...
class Sam2MatcherLightningModel(LightningModule):
    def __init__(
        ...
        if "pp" in model_cfg:
            pp_cfg = model_cfg.pop("pp")
            model_cfg.setdefault("memory_bank_cfg", {}).setdefault("pp", {})
            for key, value in pp_cfg.items():
                if key in {"enable", "use_whitening", "use_subprototypes"}:
                    model_cfg["memory_bank_cfg"]["pp"][key] = _str2bool(value)
                elif key in {"top_r"}:
                    model_cfg["memory_bank_cfg"]["pp"][key] = int(value)
                elif key in {"alpha", "beta"}:
                    model_cfg["memory_bank_cfg"]["pp"][key] = float(value)
                elif key == "cluster" and isinstance(value, dict):
                    cluster_cfg = model_cfg["memory_bank_cfg"]["pp"].setdefault("cluster", {})
                    for c_key, c_val in value.items():
                        cluster_cfg[c_key] = int(c_val) if c_key in {"min_size", "max_k"} else c_val
                elif key == "clip" and isinstance(value, dict):
                    clip_cfg = model_cfg["memory_bank_cfg"]["pp"].setdefault("clip", {})
                    for c_key, c_val in value.items():
                        if c_key == "enable":
                            clip_cfg[c_key] = _str2bool(c_val)
                        else:
                            clip_cfg[c_key] = c_val
                else:
                    model_cfg["memory_bank_cfg"]["pp"][key] = value
    ...
    def _output_inqueue(self, output_dict):
        ...
        out_format = getattr(self.seg_model, "eval_out_format", "mask")
        encoded_results_per_img = self.eval_dataset.encode_results([results_per_img], out_format=out_format)
