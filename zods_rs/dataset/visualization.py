...
def vis_coco(gt_bboxes, gt_labels, gt_masks, scores, labels, bboxes, masks, score_thr, img_path, out_path, show_scores=False, class_names=None, dataset_name='COCO'):
    ...
    if class_names is None:
        if dataset_name == 'coco':
            ...
            # For custom datasets, use provided class_names or default to generic names
            if class_names is None:
                class_names = [f"class_{i}" for i in range(max(max(labels, default=0), max(gt_labels, default=0)) + 1)]
    ...
    if not show_scores:
        ...
        label_strs = [
            class_names[ind]+'=%d'%(round(s*100)) for ind, s in zip(class_inds, scores)
        ]
    ...
    # 确保图像是 RGB 格式（防止 RGBA 导致的通道不匹配）
    if vis_np.ndim == 3 and vis_np.shape[2] == 4:
        vis_np = vis_np[:, :, :3]  # RGBA → RGB
    if gt_vis_np.ndim == 3 and gt_vis_np.shape[2] == 4:
        gt_vis_np = gt_vis_np[:, :, :3]  # RGBA → RGB
